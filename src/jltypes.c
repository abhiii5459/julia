...
static jl_value_t *lookup_type(jl_typename_t *tn, jl_value_t **key, size_t n)
{
    ...
    size_t cl;
    if (jl_is_svec(cache)) {
        data = jl_svec_data(cache);
        cl = jl_svec_len(cache);
    }
    else {
...
static void cache_type_(jl_value_t *type)
{
    ...
    jl_svec_t *t = ((jl_datatype_t*)type)->parameters;
    if (jl_svec_len(t) == 0) return;
    ...
    if (jl_array_any_type != NULL) {
        if (jl_is_svec(cache)) {
            jl_array_t *nc = jl_alloc_cell_1d(jl_svec_len(cache));
            memcpy(nc->data, jl_svec_data(cache), sizeof(void*)*jl_svec_len(cache));
            cache = (jl_value_t*)nc;
...
void jl_init_types(void)
{
    ...
    jl_method_type =
        jl_new_datatype(jl_symbol("Method"), jl_any_type, jl_emptysvec,
                        ...
                        jl_svec(7, jl_simplevector_type, jl_bool_type, jl_bool_type,
